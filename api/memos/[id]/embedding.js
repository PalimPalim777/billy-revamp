...
function getMemoId(req) {
  ...
  const m = url.pathname.match(/\/api\/memos\/([^/]+)\/embedding/);
...
module.exports = async function handler(req, res) {
  ...
  if (!isB64(body.embedding_ciphertext)) { badInput(res, 'embedding_ciphertext'); return; }
  if (!isB64(body.embedding_iv)) { badInput(res, 'embedding_iv'); return; }
  if (typeof body.embedding_model_version !== 'string' || body.embedding_model_version.length === 0) {
    badInput(res, 'embedding_model_version');
    return;
  }
  ...
  const { data, error } = await supabase
    .from('memos')
    .update({
      embedding_ciphertext: body.embedding_ciphertext,
      embedding_iv: body.embedding_iv,
